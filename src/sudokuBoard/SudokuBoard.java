...
public class SudokuBoard extends JFrame implements ActionListener {
	...
	static final String ZIGZAG_AC = "Zigzag", AC_AC = "ALLCLEAR";
	...
	public SudokuBoard(int board_size) {
		...
		if (boardSize == Sudoku.BASIC_SIZE) {
			...
			for (int i = 0; i < Sudoku.BASIC_SIZE + 2; i++) {
				for (int j = 0; j < Sudoku.BASIC_SIZE + 2; j++) {
					if (i % 4 == 3)
						boardPanel.add(new JLabel(Sudoku.EMPTY_BOX));
					else if (j % 4 == 3)
						boardPanel.add(new JLabel(Sudoku.EMPTY_BOX));
					else {
		...
		evenOddModeButton.setActionCommand(ZIGZAG_AC);
		evenOddModeButton.addActionListener(this);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().startsWith(NUMBER_AC_INITIAL)) {
			...
			for (JRadioButton rb : numberBoard) {
				if (rb.getIcon().equals(eobox_selected) || rb.getIcon().equals(eobox_unselected))
					continue;
				else {
					rb.setIcon(box_unselected);
					rb.setSelectedIcon(box_selected);
			...
			validate();
		} else if (e.getActionCommand().equals(ZIGZAG_AC)) {

		} else if (e.getActionCommand().equals(OK_AC)) {
			NanpureSolver n;
			if (Sudoku.sudokuMode == Sudoku.BASIC_MODE) {
				if (basicModeButton.isSelected())
					n = new BasicNanpureSolver(board);
				else if (diagModeButton.isSelected())
					n = new DiagnoalNanpureSolver(board);
				...
				} else if (zigzagModeButton.isSelected())
					n = new ZigzagNanpureSolver(board);
				else
					n = new BasicNanpureSolver(board);
				if (solveAllCheckBox.isSelected())
					n.solveAll();
				else {
					n.solve();
				}
				...
				if (basicModeButton.isSelected())
					n = new MiniBasicNanpureSolver(board);
				else if (diagModeButton.isSelected())
					n = new MiniDiagnoalNanpureSolver(board);
				/*
				else if (evenOddModeButton.isSelected()) {
					String[] newBoard = new String[boardSize*boardSize];
					for (int i=0; i < boardSize*boardSize; i++) {
